...
temperatures = [22, 35, 19, 40, 25, 30]  # Example list of temperatures
threshold = 30  # Threshold temperature

for temp in temperatures:
    if temp > threshold:
        print(f"Warning: The temperature {temp}°C exceeds the threshold!")
    else:
        print(f"The temperature {temp}°C is within the safe limit.")
...
# Check stock levels and generate restock alerts
for product, details in products.items():
    if details["stock"] < details["min_required"]:
        print(f"Alert: {product} needs restocking! Current stock: {details['stock']}, Minimum required: {details['min_required']}")
    else:
        print(f"{product} stock is sufficient. Current stock: {details['stock']}, Minimum required: {details['min_required']}")
